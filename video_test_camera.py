...
def init_det_and_emb_model(model_file):
    ...
    if model_file is not None:
        ...
        custom_objects = {
            "function": your_custom_activation_function  # Thay "function" và your_custom_activation_function bằng tên và hàm thực tế
        }
        
        # Tải mô hình với `custom_objects`
        face_model = tf.keras.models.load_model(model_file, custom_objects=custom_objects, compile=False)
    else:
        face_model = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", "--model_file", type=str, required=True, help="Path to the saved model file.")
    parser.add_argument("-k", "--known_user", type=str, default=None, help="Folder containing user images.")
    parser.add_argument("-K", "--known_user_force", type=str, default=None, help="Force reload user images data.")
    parser.add_argument("-b", "--embedding_batch_size", type=int, default=4, help="Batch size for embedding data.")
    parser.add_argument("-s", "--video_source", type=str, default="0", help="Video source; '0' for default camera.")
    parser.add_argument("-t", "--dist_thresh", type=float, default=0.6, help="Threshold for unknown faces.")
    parser.add_argument("-p", "--frames_per_detect", type=int, default=5, help="Frames per detection.")
    args = parser.parse_args()

    # Khởi tạo mô hình và bộ phát hiện
    det, face_model = init_det_and_emb_model(args.model_file)

    # Kiểm tra thư mục ảnh người dùng
    known_user = args.known_user_force if args.known_user_force is not None else args.known_user
    force_reload = args.known_user_force is not None

    # Nhúng dữ liệu người dùng đã biết nếu có thư mục ảnh người dùng
    if known_user and face_model:
        image_classes, embeddings, _ = embedding_images(
            det, face_model, known_user, args.embedding_batch_size, force_reload
        )

        # Thiết lập nguồn video (sử dụng camera nếu là số)
        ...
        print("Vui lòng cung cấp thư mục ảnh người dùng và đường dẫn tệp mô hình hợp lệ.")
...
# camera
# python video_test_camera.py -m D:/CODE/PYTHON/Do_an/v2/GhostFaceNets/checkpoints/ghostnetv2_w1.3_s2_basic_agedb_30_epoch_45_0.975000.h5 -s 0

...
class FaceDetectorAndRecognizer:
    def __init__(self, recognize_model="EfficientNetV2B0", known_users=None, batch_size=32, force_reload=False):
        if recognize_model == "EfficientNetV2B0":
            recognize_model_path = "D:/CODE/PYTHON/Do_an/v2/GhostFaceNets/checkpoints/ghostnetv1_w1.3_s2_new_data_(8file_githud)_basic_agedb_30_epoch_1_0.560333.h5"
            if not os.path.exists(recognize_model_path):
                raise FileNotFoundError(f"Model file not found at {recognize_model_path}")
            def hard_sigmoid_torch(x):
                return tf.clip_by_value((x + 1) / 2, 0, 1)
            self.recognize_model = tf.keras.models.load_model(
                recognize_model_path, 
                custom_objects={"kecamCommon>hard_sigmoid_torch": hard_sigmoid_torch}, 
                compile=False
            )
...
if __name__ == "__main__":
    ...
    # Thiết lập các tham số cho argparse
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("-m", "--recognize_model", default="EfficientNetV2B0", help="Saved model file path, NOT model")
    parser.add_argument("-k", "--known_users", type=str, default=None, help="Folder containing user images data")
    ...
    parser.add_argument("-v", "--video_source", type=str, default="0", help="Video source (0 for default camera)")
    parser.add_argument("-i", "--images", nargs="*", type=str, help="Image file paths to compare")
    parser.add_argument("-t", "--similarity_thresh", type=float, default=0.6, help="Cosine similarity threshold")
    parser.add_argument("-p", "--frames_per_detect", type=int, default=5, help="Detect every [NUM] frames")
...
# python test_camera.py --images D:/CODE/PYTHON/Do_an/v2/GhostFaceNets/213.jpg


# python test_camera.py --images test.jpg --known_users test

# python test_camera.py --known_users test --video_source 0
